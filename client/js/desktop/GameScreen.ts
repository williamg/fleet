...
export class GameScreen extends Scene {
    ...
    public enter(stage: PIXI.Container, callback: () => void): void {
        this._grid = new Grid(this._grid_system, this._input_handler);
        this._grid.x = 1920 / 2;
        this._grid.y = 1080 / 2;

        this._target_window = new TargetWindow(this._input_handler, this._state, this._friendly);
        this._target_window.x = 0;
        this._target_window.y = 1080 - this._target_window.height;

        this._hanger_window =
            new HangerWindow(this._input_handler, this._hanger_system,
